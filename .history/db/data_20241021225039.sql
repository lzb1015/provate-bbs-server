...
-- 添加留言数据
CREATE TABLE IF NOT EXISTS `leavel`(`lid` VARCHAR(40)  NOT NULL,`uid` VARCHAR(40) ,`tid` VARCHAR(40),`value` VARCHAR(1000) NOT NULL ,`index` INT,`createTime` VARCHAR(100),`like` VARCHAR(1000),`plid` VARCHAR(40),PRIMARY KEY (`lid`) ,FOREIGN KEY(`uid`) REFERENCES user(`id`) ON DELETE CASCADE,FOREIGN KEY(`tid`) REFERENCES posts(`tid`) ON DELETE CASCADE,FOREIGN KEY (`plid`) REFERENCES leavel(`lid`) ON DELETE CASCADE)ENGINE=InnoDB DEFAULT CHARSET=utf8;
