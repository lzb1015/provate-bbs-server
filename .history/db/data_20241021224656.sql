...
CREATE TABLE IF NOT EXISTS `platen`(`cid` VARCHAR(40) NOT NULL,`name` VARCHAR(100) NOT NULL,`parent_id` VARCHAR(40) UNSIGNED,PRIMARY KEY (`cid`),FOREIGN KEY(`parent_id`) REFERENCES platem(`pid`) ON DELETE CASCADE)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
-- 创建帖子数据表
CREATE TABLE IF NOT EXISTS `posts`(`tid` VARCHAR(40) NOT NULL,`title` VARCHAR(200) NOT NULL,`abstract` TEXT,`content` TEXT NOT NULL,`create_time` VARCHAR(100),`like` VARCHAR(1000) default '[]',`tImg` VARCHAR(1000),`pid` VARCHAR(40) UNSIGNED,`cid` VARCHAR(40) UNSIGNED,`uid` VARCHAR(40) UNSIGNED,PRIMARY KEY (`tid`),FOREIGN KEY (`uid`) REFERENCES user(`id`) ON DELETE CASCADE,FOREIGN KEY (`pid`) REFERENCES platem(`pid`) ON DELETE CASCADE,FOREIGN KEY (`cid`) REFERENCES platen(`cid`) ON DELETE CASCADE)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
-- 添加留言数据
CREATE TABLE IF NOT EXISTS `leavel`(`lid` VARCHAR(40)  NOT NULL,`uid` VARCHAR(40) ,`tid` VARCHAR(40) UNSIGNED,`value` VARCHAR(1000) NOT NULL ,`index` INT,`createTime` VARCHAR(100),`like` VARCHAR(1000),`plid` VARCHAR(40) UNSIGNED,PRIMARY KEY (`lid`) ,FOREIGN KEY(`uid`) REFERENCES user(`id`) ON DELETE CASCADE,FOREIGN KEY(`tid`) REFERENCES posts(`tid`) ON DELETE CASCADE,FOREIGN KEY (`plid`) REFERENCES leavel(`lid`) ON DELETE CASCADE)ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
-- -- 添加消息数据
CREATE TABLE IF NOT EXISTS `message`(`mid` VARCHAR(40)  NOT NULL,`uid` VARCHAR(40) ,`wid` VARCHAR(40) UNSIGNED,`lid` VARCHAR(40) ,`type` VARCHAR(30) NOT NULL,`tid` VARCHAR(40),`isread` INT,PRIMARY KEY(`mid`),FOREIGN KEY(`uid`) REFERENCES user(`id`) ON DELETE CASCADE,FOREIGN KEY(`wid`) REFERENCES user(`id`) ON DELETE CASCADE,FOREIGN KEY(`tid`) REFERENCES posts(`tid`) ON DELETE CASCADE,FOREIGN KEY(`lid`) REFERENCES leavel(`lid`) ON DELETE CASCADE)ENGINE=InnoDB DEFAULT CHARSET=utf8;
