...
// 删除帖子
async function del_db_post(tid, uid){
    let result = await new Promise((resolve,reject)=>{
        connection.query('DELETE FROM posts WHERE tid = ? and uid = ?',[tid, uid],(err,res)=>{
            // console.log(res,err)
            if(err) reject(err)
            resolve(res)
        })
...
// 设置用户最后登陆时间
async function set_last_login_time_db(obj){
    let res = await new Promise((resolve,reject)=>{
        connection.query('UPDATE FROM USER SET lastLoginTime = ? WHERE id = ?',[obj.time, obj.uid],(res,err)=>{
            if(err) reject(err)
            resolve(res)
        })
    })
