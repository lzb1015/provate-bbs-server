...
async function add_new_post(obj){
    // console.log(obj)
    let result = await new Promise((resolve,reject)=>{
        connection.query("INSERT INTO posts(tid,title,abstract,content,create_time,`like`,tImg,pid,cid,uid)VALUES(?,?,?,?,?,?,?,?,?,?);",[obj.tid,obj.title,obj.abstract,obj.content,obj['create_time'],JSON.stringify(obj.like),obj.tImg,obj.pid,obj.cid,obj.uid],(err)=>{
            if(err){
                reject(err)
            }
            resolve(res)
...
async function add_db_leavel(list){
    let result = await new Promise((resolve,reject)=>{
        connection.query('INSERT INTO leavel VALUES(?,?,?,?,?,?,?,?)',list,(err,res)=>{
            if(err){
                reject(err)
            }
            resolve(res)
