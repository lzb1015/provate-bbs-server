...
// 对留言变化成Object
async function changeLeavel(list,index,plid = null){
    let result = []
    let flag = false
    list.forEach(item=>{
        if(item.index === index){
    ...
    for(item of list){
        if(item.index === index && item.plid === plid){
            item.like = JSON.parse(item.like)
            // 获取留言用户的数据
            let leavel_user = await get_user_info_id(item.uid)
            item.nickName = leavel_user.nickName
            item.usericon = leavel_user.icon
            result.push(item)
        }
    }
    index += 1
    for(const item of result){
        item.children = await changeLeavel(list,index,item.lid)
    }
    return result
}
...
router.get('/get_post_detail',async(req,res)=>{
    ...
    // 获取留言树
    let newLeavel = await changeLeavel(leavel,-1)
    // console.log(newLeavel)
    newLeavel = newLeavel.filter(item=>item.tid === tid)
...
router.post('/add_post',async(req,res)=>{
    ...
    try{
        ...
        res.status(500).send('add post is error')
    }
...
router.post('/del_post',async(req,res)=>{
    ...
    await del_db_post(req.body.tid, req.body.uid)
