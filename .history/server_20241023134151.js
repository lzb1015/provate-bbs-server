...
app.post('/login',async (req,res)=>{
    ...
    if(!result || result.length == 0){
        res.status(500).send('your name or password is error!')
        return
    }
...
app.post('/set_last_login_time', async(req,res)=>{
    let result = await set_last_login_time_db(req.body.uid, req.body.time)
    res.send(result)
})
